...
Matrix::Matrix() {}
Matrix::Matrix(long nr_rs, int nr_us, int nr_is, float avg) : nr_us(nr_us), nr_is(nr_is), nr_rs(nr_rs), avg(avg) { M = new Node[nr_rs]; }
...
void Model::initialize(Matrix *Tr) {
    ...
    srand48(0L);
    P = new float[(long)nr_us*dim_off]; float *p = P;
    ...
    srand48(0L);
    Q = new float[(long)nr_is*dim_off]; float *q = Q;
...
void Model::read(char *path) {
    ...
    FILE *f = fopen(path,"rb"); if(!f) exit_file_error(path);
    read_meta(f);
    P = new float[(long)nr_us*dim_off]; Q = new float[(long)nr_is*dim_off];
    fread(P, sizeof(float), (long)nr_us*dim_off, f);
    fread(Q, sizeof(float), (long)nr_is*dim_off, f);
	if(en_ub) {
		UB = new float[nr_us];
...
void Model::write(char *path) {
    ...
    fwrite(P,sizeof(float),(long)nr_us*dim_off,f);
    fwrite(Q,sizeof(float),(long)nr_is*dim_off,f);
...
void Model::shuffle() {
	float *P1 = new float[(long)nr_us*dim_off]; float *Q1 = new float[(long)nr_is*dim_off]; float *UB1 = new float[nr_us]; float *IB1 = new float[nr_is];
	for(int px=0; px<nr_us; px++) std::copy(&P[(long)px*dim_off],&P[(long)px*dim_off+dim_off],&P1[(long)map_uf[px]*dim_off]);
	for(int qx=0; qx<nr_is; qx++) std::copy(&Q[(long)qx*dim_off],&Q[(long)qx*dim_off+dim_off],&Q1[(long)map_if[qx]*dim_off]);
...
void Model::inv_shuffle() {
	float *P1 = new float[(long)nr_us*dim_off]; float *Q1 = new float[(long)nr_is*dim_off]; float *UB1 = new float[nr_us]; float *IB1 = new float[nr_is];
	for(int px=0; px<nr_us; px++) std::copy(&P[(long)px*dim_off],&P[(long)px*dim_off+dim_off],&P1[(long)map_ub[px]*dim_off]);
	for(int qx=0; qx<nr_is; qx++) std::copy(&Q[(long)qx*dim_off],&Q[(long)qx*dim_off+dim_off],&Q1[(long)map_ib[qx]*dim_off]);
	delete [] P; delete [] Q; P = P1; Q = Q1;
    if(en_ub) {
...
float calc_rate(Model *model, Node *r) { 
    float rate = std::inner_product(&model->P[(long)r->uid*model->dim_off], &model->P[(long)r->uid*model->dim_off]+model->dim, &model->Q[(long)r->iid*model->dim_off], 0.0) + model->avg; 
