...
void sgd(GridMatrix *TrG, Model *model, Scheduler *scheduler, int tid) {
    ...
    while(true) {
		...
		rn = TrG->GMS[jid]->M; nr_rs = TrG->GMS[jid]->nr_rs; pn = P + (long)rn->uid*dim; qn = Q + (long)rn->iid*dim; ubn = UB + rn->uid; ibn = IB + rn->iid;
    	XMMl = _mm_setzero_pd();
		for(mx=0; mx<nr_rs; mx++) {
			r = rn; rn++; __m128 XMMr = _mm_load1_ps(&r->rate), XMMge = _mm_setzero_ps(), XMMub, XMMib;
			p = pn; pn = P + (long)rn->uid*dim;
			q = qn; qn = Q + (long)rn->iid*dim;
			...
			if((mx+15)<nr_rs) {
				...
				_mm_prefetch(qn,_MM_HINT_T0); _mm_prefetch(Q+(rn+7)->iid*(long)dim,_MM_HINT_T1); _mm_prefetch(Q+(rn+15)->iid*(long)dim,_MM_HINT_T2);
				_mm_prefetch(pn,_MM_HINT_T0); _mm_prefetch(P+(rn+7)->uid*(long)dim,_MM_HINT_T1); _mm_prefetch(P+(rn+15)->uid*(long)dim,_MM_HINT_T2);
			}
			for(dx=0; dx<dim-7; dx+=8) {
