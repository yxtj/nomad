...
hugewiki_timeouts = [5000, 10000, 20000, 40000, 60000, 80000, 100000, 120000, 140000, 160000, 180000, 200000, 220000, 240000, 260000, 280000, 300000]

datasets = [("yahoo", 1.00, 0.0005, 0.05, default_timeouts), 
            ("netflix", 0.05, 0.008, 0.01, default_timeouts),
            ("hugewiki", 0.01, 0.00066666, 0, hugewiki_timeouts)]
...
for dataset, par_reg, lrate, drate, timeouts in datasets:
    for numcpu in numcpus:
        ...
        ofile.write("ibrun ../../../Code/nomad/nomad_float --timeout %s --path ../../../Data/%s --dim %d --reg %f --lrate %f --drate %f --nthreads %d > %s \n" % (timeout_str, dataset, dim, par_reg, lrate, drate, numcpu, log_fname) )
